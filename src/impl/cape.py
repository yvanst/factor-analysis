...
class CapeImpl:
    ...
    def get_security_list(self, observe_date):
        ...
        security_list = self.sort_security_using_z_score(total_signal_df)
        # less PE is better, thus we reverse the order
        security_list = list(reversed(security_list))
        assert len(security_list) > 0
        return security_list

    def sort_security_using_z_score(self, total_signal_df):
        """
        sort signal in descending order
        """
        latest_month = (
        ...
        merge_df = latest_signal_df.join(
            total_signal_df, on="sedol7", how="inner"
        ).with_columns(
    ...
    def get_security_signal(self, date):
        ...
        price_df = (
            ...
            .filter(pl.col("price") > 5)  # we don't like cheap stocks
            .collect()
        )
